...
//const { sendConfirmationEmail } = require('nodemailer')
    ...
    //pub_caty_now             
    //           ____login_________
    app.post('/'+ my_link +'/pub_caty_now', isLoggedIn , (req, res) => {
        conn.connect((err) => {
            var sql = "INSERT INTO catygories_chosen_ (name) VALUES ('"+req.body.Nmm1+"')"
            conn.query(sql, function (err1, result) {
                if (err1) console.log(err1);
            });
        });
    ...
    /// editt catyy             
    //           ____login_________
    app.post('/'+ my_link +'/caty_edit/:caty_edit', isLoggedIn, (req, res) => {
        conn.connect((err) => {
            var sql = "UPDATE catygories_chosen_ SET name = '"+req.body.Nmm2+"'  WHERE catygories_chosen_.id = "+req.params.caty_edit
            conn.query(sql, function (err1, result) {
            if (err1) console.log(err1);
            });
        });
